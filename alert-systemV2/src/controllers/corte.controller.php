...
class CorteController {
    ...
    //Funcion para obtener el id y nombre del corte para mostrarlos en la vista Ver de Detalles del corte
    function obtenerDatosCorte() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_POST['id']) && isset($_POST['nombre'])) {
                $idCorte = $_POST['id'];
                $nombre = $_POST['nombre'];
                echo 'Detalles del corte con ID ' . $idCorte . ': ' . $nombre;
                echo '<h2 class="card-title text-center">Detalles del corte con ID: '. $idCorte . ': ' . $nombre . '</h2>';
